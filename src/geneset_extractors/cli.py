...
def _add_igvf_perturbseq_flags(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--expression_tsv", required=True)
    parser.add_argument("--source_input_id", default="expression_tsv")
    parser.add_argument("--mapping_file")
    parser.add_argument("--out_dir", required=True)
    parser.add_argument("--organism", choices=["human", "mouse"], default="human")
    parser.add_argument("--genome_build", default="hg38")
    parser.add_argument("--orientation", choices=["perturbation_by_gene", "gene_by_perturbation"], default="perturbation_by_gene")
    parser.add_argument("--gmt_name", default="gene_set_library_crisp.gmt")
    parser.add_argument("--z_threshold", type=float, default=3.0)
    parser.add_argument("--min_gmt_size", type=int, default=5)
    parser.add_argument("--input_mode", choices=["matrix", "long_de"], default="matrix")
    parser.add_argument("--sep", default="auto")
    parser.add_argument("--term_column")
    parser.add_argument("--gene_symbol_column")
    parser.add_argument("--gene_id_column")
    parser.add_argument("--effect_column")
    parser.add_argument("--ratio_column")
    parser.add_argument("--score_column")
    parser.add_argument("--pvalue_column")
    parser.add_argument("--pvalue_max", type=float)
    parser.add_argument("--score_threshold", type=float)
    parser.add_argument("--top_k_per_direction", type=int)
    _add_provenance_flags(parser)
    ...
    try:
        ...
        if args.command == "workflows":
            ...
            if args.workflow_command == "igvf_perturbseq":
                ...
                print(f"workflow_completed workflow=igvf_perturbseq n_rows={result.get('n_rows')} out={result.get('out_dir')}", file=sys.stderr)
                return 0
