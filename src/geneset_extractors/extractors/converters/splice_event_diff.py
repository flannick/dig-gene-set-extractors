...
def _infer_source_dataset(rows: list[SpliceRow]) -> str | None:
    for candidate in ("source_dataset", "source_study_id", "study_id"):
        values = sorted({_clean(row.values.get(candidate)) for row in rows if _clean(row.values.get(candidate))})
        if len(values) == 1:
            return values[0]
    return None
...
def run(args) -> dict[str, object]:
    ...
    if use_bundle and gene_burden_resource_id and ctx is not None:
        gene_burden_path = resolve_resource_path(
            ...
            enablement_hint=(
                "Provide --resources_dir containing the splice gene-burden table, or set --gene_burden_resource_id "
                "to a valid local resource."
            ),
        )
