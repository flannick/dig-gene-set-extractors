...
def run(args) -> dict[str, object]:
    activate_runtime_context("methylation_dmr", getattr(args, "provenance_overlay_json", None))
    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    raw = _read_scores(args.dmr_tsv, args.gene_id_column, args.score_column)
    ...
    meta = make_metadata(
        converter_name="methylation_dmr",
        parameters={
        ...
        files=[input_file_record(args.dmr_tsv, "dmr_tsv")],
