...
from geneset_extractors.core.atac_programs import (
...
from geneset_extractors.io.bed import read_bed, read_peak_weights_tsv
from geneset_extractors.io.gtf import read_genes_from_gtf
from geneset_extractors.io.reference_tables import (
...
def _warn_peak_weight_sanity(peak_weights: list[float], source: str) -> None:
    if not peak_weights:
        return
    n = len(peak_weights)
    unique_count = len(set(float(w) for w in peak_weights))
    vals = [float(w) for w in peak_weights]
    vals_set = set(vals)
    if vals_set.issubset({0.0, 1.0}):
        print(
            "warning: peak weights are all binary (0/1); verify weight source "
            f"({source}) and --peaks_weight_column if needed.",
            file=sys.stderr,
        )
        return
    if n >= 100 and unique_count <= 3:
        print(
            "warning: peak weights have very low cardinality; check that "
            f"{source} points to a quantitative signal column.",
            file=sys.stderr,
        )
        return
    if n >= 100 and all(abs(v - round(v)) < 1e-12 for v in vals):
        ordered = sorted(vals)
        q95 = ordered[min(len(ordered) - 1, int(0.95 * len(ordered)))]
        if q95 <= 100.0:
            print(
                "warning: peak weights are integer-valued with low dynamic range; "
                "confirm this is the intended accessibility signal.",
                file=sys.stderr,
            )
...
def run(args) -> dict[str, object]:
    ...
    peak_weight_source = "peak_weights_tsv" if args.peak_weights_tsv else f"peaks column {args.peaks_weight_column}"
    _warn_peak_weight_sanity(peak_weights, peak_weight_source)
    ...
    def _linked_scores_for_peak_values(cur_peak_values: list[float]) -> dict[str, dict[str, float]]:
        ...
        for method in link_methods:
            raw_scores = score_genes(cur_peak_values, links_by_method[method], peak_weight_transform)
    ...
    def _family_scores_for_peak_values(cur_peak_values: list[float]) -> dict[str, dict[str, dict[str, float]]]:
        ...
        for method in link_methods:
            ...
            if need_promoter:
                promoter_peak_values = mask_peak_weights(cur_peak_values, include_indices=promoter_indices)
                promoter_raw = score_genes(promoter_peak_values, links_by_method[method], peak_weight_transform)
                promoter_scores = {g: float(s) for g, s in promoter_raw.items() if float(s) != 0.0}
            if need_distal:
                distal_peak_values = mask_peak_weights(cur_peak_values, exclude_indices=promoter_indices)
                distal_raw = score_genes(distal_peak_values, links_by_method[method], peak_weight_transform)
    ...
    linked_scores_none_by_method = _linked_scores_for_peak_values(peak_weights)
    family_scores_none_by_method = _family_scores_for_peak_values(peak_weights)
    ...
    if ref_peak_idf is not None:
        adjusted_values = [float(w) * float(i) for w, i in zip(peak_weights, ref_peak_idf)]
        ref_summary = {
