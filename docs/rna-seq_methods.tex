...
\title{Supplementary Note: RNA-seq Differential Expression to Gene Programs and Gene Sets in dig-gene-set-extractors}
\author{}
\date{}
...
This note specifies the RNA-seq differential expression (DE) extractors currently implemented as \texttt{rna\_deg} (single contrast) and \texttt{rna\_deg\_multi} (batched contrasts from one long table). In contrast to ATAC-seq, RNA-seq is already gene-centric: the primary measured objects are genes (or transcripts), so the central modeling step is not locus-to-gene projection, but rather how to:
...
This note is designed to cross-reference the CLI documentation in \texttt{docs/rna-seq2geneset.md}.
...
\item Gene set export: \texttt{--emit\_gmt}, \texttt{--gmt\_topk\_list}, \texttt{--gmt\_split\_signed}, \texttt{--gmt\_source}.
...
\item \texttt{emit\_gmt=true}, \texttt{gmt\_topk\_list=200}, \texttt{gmt\_split\_signed=true}
\item Conservative artifact filters enabled but user-configurable
\item Strongly recommend setting \texttt{signature\_name} for provenance
\end{itemize}
