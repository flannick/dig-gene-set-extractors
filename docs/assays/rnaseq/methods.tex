...
This note specifies the RNA-seq extractors currently implemented as \texttt{rna\_deg} (single contrast), \texttt{rna\_deg\_multi} (batched contrasts from one long table), and \texttt{rna\_sc\_programs} (external scRNA factor-loading ingestion). Upstream RNA preparation and inference now live under workflow/preprocessing commands (for example \texttt{workflows rna\_de\_prepare} and \texttt{workflows scrna\_cnmf\_prepare}), while the extractor layer remains responsible for converting already-scored results into gene programs and GMT sets. In contrast to ATAC-seq, RNA-seq is already gene-centric: the primary measured objects are genes (or transcripts), so the central modeling step is not locus-to-gene projection, but rather how to:
...
\item Optional pre-aggregation row filters: \texttt{--padj\_max}, \texttt{--pvalue\_max}, \texttt{--min\_abs\_logfc}.
...
\subsection{S1.2b: \texttt{score\_mode=signed\_neglog10padj}}
When adjusted p-values are the primary confidence scale but direction should still come from the fitted effect, the extractor supports:
\begin{equation}
s_g = \operatorname{sign}(u_g) \cdot \min\{-\log_{10}(q_g + \epsilon), C\},
\label{eq:signed_neglog10padj}
\end{equation}
where $u_g$ is taken from the signed statistic $t_g$ if available, otherwise from $\ell_g$.
This mode is useful for workflows that want FDR-driven magnitude while keeping up/down direction explicit.
...
\subsection{S1.5: pre-aggregation row filters}
Before duplicate collapse and gene aggregation, the extractor can optionally apply row-level thresholds:
\begin{itemize}[leftmargin=2em]
\item \texttt{--padj\_max}: keep only rows with adjusted p-value $\leq q_{\max}$.
\item \texttt{--pvalue\_max}: keep only rows with raw p-value $\leq p_{\max}$.
\item \texttt{--min\_abs\_logfc}: keep only rows with $|\ell_g| \geq \tau$.
\end{itemize}
These filters operate on the raw DE table and are shared by both \texttt{rna\_deg} and \texttt{rna\_deg\_multi}.
...
\section{Workflow-layer DE preparation}
The workflow command \texttt{geneset-extractors workflows rna\_de\_prepare} exists upstream of the extractors.
Its role is to convert count matrices plus metadata into a standardized long DE table with columns such as:
\begin{itemize}[leftmargin=2em]
\item \texttt{comparison\_id}
\item \texttt{gene\_id}
...
\item \texttt{padj}
\end{itemize}
This boundary is intentional: raw DE fitting is treated as preprocessing/inference, while \texttt{rna\_deg} and \texttt{rna\_deg\_multi} remain assay-result converters.

\subsection*{Bulk mode}
For bulk RNA-seq, the workflow consumes a dense TSV count matrix plus sample metadata, optionally joined with subject-level metadata.
It supports simple CLI-generated contrasts (two-group, group-vs-rest, reference-level within stratum) or an explicit comparisons table.

\subsection*{DE preparation modes}
The preprocessing layer now exposes explicit DE-fitting modes rather than overloading extractor post-processing presets.

\texttt{de\_mode=modern} is the general-purpose default: all eligible samples for each contrast are retained, and covariates, batch terms, or repeated-measures designs may be used subject to backend support.

\texttt{de\_mode=harmonizome} is a narrow bulk-RNA preset intended to more closely reproduce the published GTEx/Harmonizome aging notebook. For each two-group comparison, let $A$ and $B$ denote the eligible sample sets after stratification and group filtering. The balanced analysis set is
\[
\tilde{A} \subseteq A,\qquad \tilde{B} \subseteq B,\qquad |\tilde{A}| = |\tilde{B}| = \min(|A|, |B|).
\]
The downsampling is deterministic given a base seed and comparison identifier, and the selected sample IDs are written to audit artifacts. The preset still allows explicit fixed-effect covariates, which is useful in broad tissues such as GTEx aging where balancing alone is often not sufficient to suppress generic mitochondrial or housekeeping signatures. In practice we recommend explicit covariates such as \texttt{SEX} and \texttt{SMTSD} when those columns are available. Batch terms and repeated-measures flags remain disallowed so the fit stays auditable and narrow in scope.

\subsection*{scRNA pseudobulk mode}
For scRNA-seq, the workflow defaults to donor-level pseudobulk rather than naive per-cell DE.
If cell type is provided, pseudobulks can be formed within cell type and then compared across conditions.
This yields DE tables that the extractor layer can consume in the same way as bulk-derived contrasts.

\subsection*{Lightweight backend limitations}
The built-in \texttt{lightweight} backend is intended for simple, dependency-light first passes.
It uses low-expression filtering, library-size normalization, log-scale modeling, and BH correction within each emitted contrast, but it remains approximate:
\begin{itemize}[leftmargin=2em]
\item no true random-effects inference
\item intended for fixed-effect designs
\item not a replacement for limma-voom or dream on complex repeated-measures studies
\end{itemize}
Therefore repeated-measures requests do not silently fall back to the lightweight backend; the user must explicitly allow that approximation.
