...
If event $i$ is only available through a low-confidence cohort-local raw identifier, then the prior contribution is shrunk more strongly toward neutrality. A simple v1 rule is
\[
u_i^{\star} = 1 + \lambda_i (u_i - 1),
\]
with $\lambda_i = 1$ for coordinate-canonical events and $\lambda_i \ll 1$ for \texttt{raw\_id\_fallback} events.
...
\begin{itemize}[leftmargin=2em]
\item the current input after duplicate collapse, or
\item a compact reference-bundle table such as \texttt{splice\_gene\_event\_burden\_human\_v1.tsv.gz}.
\end{itemize}

The goal is not to erase event-rich genes, only to stop them from winning by event-count opportunity alone.
