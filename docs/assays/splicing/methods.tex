...
\item \texttt{workflows splice\_prepare\_reference\_bundle}: builder for a lightweight event alias, ubiquity, conservative impact, and gene-burden bundle.
\end{itemize}
...
\item Reference bundle:
  \texttt{--resources\_dir}, \texttt{--resources\_manifest}, \texttt{--resource\_policy}, \texttt{--use\_reference\_bundle}, and resource ids for event alias, ubiquity, impact, and gene-burden priors.
...
\end{itemize}
For \texttt{splice\_event\_matrix}, per-event p-values are Benjamini-Hochberg adjusted within each emitted contrast before this resolver runs, so \texttt{score\_mode=auto} preferentially consumes a contrast-local \texttt{padj} when one can be computed.
...
If event $i$ is only available through a low-confidence cohort-local raw identifier, then the ubiquity prior is neutralized:
\[
u_i^{\star} =
\begin{cases}
u_i & \text{high-confidence coordinate-canonical event},\\
1 & \text{\texttt{raw\_id\_fallback} / low-confidence event.}
\end{cases}
\]
...
\end{cases}
\]
with a conservative default such as $\delta_0 = 0.05$. This downweights tiny absolute PSI shifts instead of hard-dropping them.
...
Several rows from one LeafCutter cluster, MAJIQ LSV, or similar local event family are not independent evidence. The runtime therefore defines an event-group key $c_i$ from \texttt{event\_group} when available, falling back to the canonical event key otherwise, and keeps one representative per $(g, c_i)$ before gene aggregation:
\[
E_g^{\star} = \{ \text{best representative of each } (g, c_i) \}.
\]
This representative is chosen by strongest absolute final event score, with confidence as a tiebreaker. All gene-level aggregation and current-input burden penalties operate on $E_g^{\star}$ rather than the raw post-duplicate rows.
...
Even after top-$K$ aggregation, some cohorts still favor genes with many measurable splice events. The runtime therefore applies an additional conservative multiplicity penalty:
\[
...
In the reference-bundle path, $n_g$ is derived from cross-study nuisance summaries such as unique event groups per gene, number of supporting studies, and median per-study unique-group burden. The goal is not to erase event-rich genes, only to stop them from winning by event-count opportunity alone.
...
The matrix front-end then emits one standardized per-contrast event table with columns such as \texttt{delta\_psi}, \texttt{stat}, BH-adjusted \texttt{padj}, optional support summaries, and all needed event metadata, and feeds that table to the same event-to-gene workflow as \texttt{splice\_event\_diff}.
...
The public workflow should accept raw MD Anderson style header aliases such as \texttt{symbol} for \texttt{gene\_symbol} and \texttt{as\_id} for \texttt{event\_id}. If an explicit sample annotation table is not available, a narrow fallback can infer tumor versus adjacent-normal from the \texttt{\_Norm} suffix on sample columns, recording the inference rule and any barcode-consistency checks in \texttt{prepare\_summary.json}.
...
\item canonicalization confidence counts for both retained events and selected support events,
...
\item whether low-confidence priors contributed to selected events,
\item counts of low-confidence ubiquity and impact priors that were neutralized,
...
\item \textbf{locus density artifact}: top genes concentrate within one genomic window or chromosome.
\end{itemize}
