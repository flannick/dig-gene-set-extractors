...
\item authentic wide CalR session matrices with membership columns (\texttt{group1}, \texttt{group2}, \ldots), \texttt{group\_names}, \texttt{xrange}, \texttt{light}, and \texttt{exc} are parsed as session metadata rather than naively treated as subject-level group labels,
...
For authentic wide CalR session layouts, the extractor derives subject-level session annotations by:
\begin{enumerate}[leftmargin=2em]
\item treating \texttt{group1}, \texttt{group2}, \ldots as membership columns containing subject identifiers,
\item aligning those columns to ordered labels from \texttt{group\_names},
\item deriving the selected analysis window from \texttt{xrange} and \texttt{exc},
\item recording these decisions as \texttt{session\_group\_layout\_mode} and \texttt{session\_window\_layout\_mode}.
\end{enumerate}
...
For mouse-first ontology resources, the emitted gene program is humanized by default after phenotype routing. Let $M(g_m)$ denote the set of human ortholog targets for mouse gene $g_m$. The current default uses a conservative unique-mapping policy:
\begin{equation}
S^{\mathrm{onto,human}}_{g_h}(q) = \sum_{g_m : M(g_m)=\{g_h\}} S^{\mathrm{onto}}_{g_m}(q),
\end{equation}
and ambiguous mouse genes are dropped rather than expanded by default.
...
When reference metadata already carries humanized \texttt{output\_gene\_*} fields, those identifiers are used directly. Otherwise a mouse-first reference library is humanized after retrieval with the same conservative orthology rule:
\begin{equation}
S^{\mathrm{prof,human}}_{g_h}(q) = \sum_{g_m : M(g_m)=\{g_h\}} S^{\mathrm{prof}}_{g_m}(q).
\end{equation}
...
`workflows calr_prepare_public` is the public-data staging layer for `calr_profile_query`: it takes raw local CalR-style studies plus a small study manifest, reuses the same CalR-compliant feature extraction and grouped-contrast logic as the converters, selects one reference contrast per study, and writes:
\begin{itemize}[leftmargin=2em]
\item \texttt{reference\_profiles.tsv}
\item \texttt{reference\_metadata.tsv}
\item \texttt{feature\_schema.tsv}
\item \texttt{feature\_stats.tsv}
\item \texttt{resolved\_studies.tsv}
\end{itemize}

For mouse studies, the public-preparation workflow now humanizes the reference gene label by default while preserving source provenance:
\begin{itemize}[leftmargin=2em]
\item \texttt{gene\_id}, \texttt{gene\_symbol}: emitted human identifiers
\item \texttt{source\_gene\_id}, \texttt{source\_gene\_symbol}: original mouse study labels
\item \texttt{gene\_mapping\_status}: unique, ambiguous, or source
\end{itemize}

`workflows calr_prepare_reference_bundle` then packages:
\begin{itemize}[leftmargin=2em]
...
\item optional mouse-human ortholog table for mouse bundles,
