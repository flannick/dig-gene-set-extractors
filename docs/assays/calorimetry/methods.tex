...
\texttt{workflows calr\_prepare\_public} is the public-data staging layer for \texttt{calr\_profile\_query}: it takes raw local CalR-style studies plus a small study manifest, reuses the same CalR-compliant feature extraction and grouped-contrast logic as the converters, selects one reference contrast per study, and writes:
\begin{itemize}[leftmargin=2em]
...
\texttt{workflows calr\_prepare\_reference\_bundle} then packages:
\begin{itemize}[leftmargin=2em]
