...
\title{Site-level PTM-omics extractors for DIG Gene Set Extractors}
\author{}
\date{March 2026}
...
\paragraph{Assay-type QC.}
For phosphoproteomics staging, the public-data workflow also computes residue
composition summaries from parsed rows:
\[
f_S,\; f_T,\; f_Y,\; f_K,\; f_{\mathrm{phospho}} = f_S + f_T + f_Y.
\]
These are recorded in \texttt{prepare\_summary.json} together with a dominant
residue family. The default workflow policy is to warn when
$f_{\mathrm{phospho}}$ is too low or $f_K$ is too high, because this usually
indicates that a public report belongs to a different PTM assay family rather
than to phosphoproteomics. A stricter policy may fail early instead of letting
the study enter the phospho scoring path.
...
For benchmarking stricter protection against many-site genes, the implemented
matrix front-end can also emit an additional comparison variant with a smaller
\texttt{gene\_topk\_sites} value, typically $K=1$, without changing the primary
default output.
...
\paragraph{Composition-aware QC.}
Public phosphoproteomics programs can still be dominated by sample composition
or lineage abundance shifts even when the PTM scorer is working correctly. The
implemented runtime summaries therefore include small built-in advisory marker
panels (for example erythrocyte, endothelial, immune/myeloid, and platelet)
that produce a \texttt{composition\_qc} block and a coarse
\texttt{tumor\_intrinsic\_confidence} score. These are warnings and summary
annotations, not hard filters.
...
\paragraph{Paired protein-adjustment variants.}
When a matched protein matrix is available, the implemented default is not to
silently choose one protein-adjustment setting. Instead,
\texttt{ptm\_site\_matrix} can emit paired variants of the same biological
contrast, typically
\[
\texttt{protein\_adjustment = none}
\qquad\text{and}\qquad
\texttt{protein\_adjustment = subtract},
\]
so that abundance-driven and PTM-adjusted programs can be compared directly
within the same study output directory.
...
\item \texttt{protein\_adjustment\_run\_mode = compare\_if\_protein}
...
\item public-data staging with assay-type QC enabled in warning mode
\end{itemize}
...
\texttt{--protein\_adjustment\_run\_mode} &
...
\texttt{--assay\_type\_policy}, \texttt{--min\_phospho\_like\_fraction},
\texttt{--max\_k\_fraction} & $(f_S,f_T,f_Y,f_K)$ &
Public phospho-assay QC gate \\
