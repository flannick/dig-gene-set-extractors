...
- if `expansion_decision.reason` is `compound_family_coherence_override`, the broader raw same-modality compound family vote was query-consistent and stronger than a shallow competing wrong-family pre-label explanation, so the mechanism branch chose the coherent family/mechanism label before expansion
- if `expansion_decision.reason` is `retained_query_consistent_label_fallback`, the broader compound pre-label vote drifted to a non-query-consistent label, but the narrower retained mechanism branch still carried a query-consistent family/mechanism label strongly enough to use for expansion
