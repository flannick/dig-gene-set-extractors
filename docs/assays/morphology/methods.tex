...
For direct-target retrieval, the practical implementation does not immediately map query $\rightarrow$ top references $\rightarrow$ genes. Instead it first forms a positive evidence pool over references, aggregates that evidence by routed target gene, and only then restricts the retained references to those supporting the strongest target candidates. If $T_{ug}$ denotes pooled target support, direct-target mode ranks candidate targets by $T_{ug}$ before the final reference cap is applied. This is designed to rescue weak distributed same-target support that would otherwise be lost when one off-target reference has the single highest similarity.
...
\begin{itemize}[leftmargin=2em]
\item genes present in the current bundle and carrying $(h^\star, \ell^\star)$,
...
Candidate genes are then weighted by local routed support when available, with only a small bundle-coverage prior for genes that match the label but have weak direct local evidence. This avoids the earlier broad family-union behavior and preserves the preference for the narrowest stable label.

\subsection{Compound promiscuity and optional target confidence}
Compound references can be more weakly informative than genetic references because of polypharmacology and uneven target annotation confidence. The current implementation therefore applies a mild bounded promiscuity penalty when accumulating compound-driven target support, so compounds with many annotated targets do not dominate exact-target nomination as strongly as single-target compounds. In addition, if the compound-target table carries optional columns such as \texttt{confidence}, \texttt{primary\_target}, \texttt{potency\_bucket}, or \texttt{source\_confidence}, these are folded into edge weights through a bounded multiplier. Inputs that omit these columns retain the previous behavior.
