...
In the current implementation this same-modality compound safeguard acts at two levels. First, pre-label voting eligibility is not determined solely by hubness-penalized evidence order: a protected slice of high raw-similarity same-modality compound neighbors is forced into the pre-label pool so broad coherent family support can still vote. Second, if the pre-label winner is supported by only a tiny cluster while the same-modality raw pool shows broader coherent support for another label, a coherence guard can still prefer the same-modality label rather than allowing an early wrong-family flip.
