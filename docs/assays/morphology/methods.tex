...
\item \texttt{mechanism}: family/mechanism-aware reranking and expansion,
\item \texttt{hybrid}: emit both a strict core output and a broader expanded output, while marking one as preferred for downstream use.
\end{itemize}
...
If the bundle includes optional target annotations such as \texttt{target\_family}, \texttt{target\_class}, or \texttt{mechanism\_label}, mechanism mode can estimate family-level or mechanism-level posterior support from the pooled routed gene evidence. When exact target support is weak but family support is coherent, the expanded output boosts genes carrying the supported family/mechanism labels. This yields broader, enrichment-ready outputs without pretending that exact-target recovery is stronger than it is.
...
Family/mechanism expansion is not triggered by any single winning label. The practical implementation requires the winning family or mechanism label to remain stable across both:
\begin{itemize}[leftmargin=2em]
\item a raw pooled neighborhood summary, and
\item a penalized pooled neighborhood summary after hubness/context penalties and local filtering.
\end{itemize}

Let $F^{\mathrm{raw}}_u(\ell)$ and $F^{\mathrm{pen}}_u(\ell)$ denote family-level support masses for label $\ell$ in the raw and penalized pooled neighborhoods of query $u$. Expansion is only permitted when the same label $\ell^\star$ wins both summaries and its support is not concentrated in a single weak neighbor. In practice, this means requiring multiple supporting neighbors and a minimum support fraction before boosting all genes carrying $\ell^\star$.

The gate is modality-aware:
\begin{itemize}[leftmargin=2em]
\item for \texttt{orf} and \texttt{crispr} queries, expansion additionally requires same-modality support for the winning family/mechanism label,
...
This asymmetry is intentional. It preserves ORF/CRISPR specificity while still allowing compound queries to expand through stable genetically supported family structure.
...
\item and an explicit expansion-decision record describing why expansion was allowed or suppressed.
\end{itemize}
