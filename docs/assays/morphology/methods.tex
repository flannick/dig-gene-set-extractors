...
In \texttt{hybrid} mode both branches are always computed independently. The strict branch writes \texttt{geneset.core.tsv}; the mechanism branch writes \texttt{geneset.expanded.tsv}; and the preferred output is selected afterward from branch-level confidence rather than by reusing one branch to define the other.
