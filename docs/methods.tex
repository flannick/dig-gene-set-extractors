...
\paragraph{Related but separate study-design contrast.}
For scATAC and bulk-matrix differential workflows, an additional design-level contrast (\texttt{--contrast}, e.g. group-vs-rest or condition-within-group) defines how peak statistics $x_p$ are computed before the three axes above are applied.
This appears in output naming as \texttt{\_\_contrast=...}.

\subsection{CLI-to-model crosswalk}
\label{sec:cli_crosswalk}
\begin{center}
\begin{tabular}{@{}p{3.2cm}p{4.7cm}p{6.5cm}@{}}
\toprule
CLI flag & Mathematical object & Practical meaning \\
\midrule
\texttt{--link\_method} & $L_{pg}^{(\ell)}$ & Region-to-gene linkage model on the link axis ($\ell$). \\
\texttt{--contrast} & $x_p$ construction & Study-design contrast used to compute peak statistics before calibration (group-vs-rest, condition-within-group, etc.). \\
\texttt{--contrast\_methods} & $\alpha_p^{(r)}$ or $u_g^{(m,r,\ell)}$ calibration & Contrast-method axis ($r$): peak-level calibration (\texttt{ref\_ubiquity\_penalty}) or gene-level calibration (\texttt{atlas\_residual}). \\
\texttt{--program\_preset}, \texttt{--program\_methods} & $\Psi_m(\cdot)$ & Program-family axis ($m$): linked/promoter/distal/enhancer-bias/TFIDF views. \\
\texttt{--select}, \texttt{--top\_k}, \texttt{--quantile}, \texttt{--min\_score} & $\mathcal{S}$ selection operator & Conversion from full gene-score distribution to compact program set (Section \ref{sec:set_extraction}). \\
\texttt{--normalize} & within-set weights $w_g$ & Weight normalization in selected genes (\texttt{within\_set\_l1} preferred for compact programs). \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Related study-design contrasts (upstream of this axis)}
Before contrast-method calibration, users may define a study-design contrast to produce peak statistics $x_p$:
\begin{itemize}[leftmargin=2em]
...
\end{itemize}
These design-level contrasts define the signed peak statistics that are then transformed to OPEN/CLOSE peak weights and passed through the contrast-method axis.
