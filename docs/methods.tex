...
tfidf\_group\_open & TF-IDF specificity-weighted cluster program & scATAC matrix & Yes (sc) \\
...
\subsubsection{M2.3: tfidf\_group\_open (scATAC default)}
...
\subsubsection{M2.4: phenotype\_open / phenotype\_close (bulk or sc, optional)}
...
\begin{itemize}[leftmargin=2em]
\item \textbf{bulk:} promoter\_activity, distal\_activity, enhancer\_bias (and opening/closing if signed peak statistics are provided).
\item \textbf{scATAC (grouped):} group\_vs\_rest\_open (and optionally close), tfidf\_group\_open, plus distal variants.
\end{itemize}
