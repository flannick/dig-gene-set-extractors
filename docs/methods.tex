...
\end{itemize}
Program families are not merely aliases for link choices: they can apply additional operations (for example promoter/distal peak masking or distal-vs-promoter combination) before/after projection, and are then evaluated for each selected link model.
...
In the CLI implementation, ATAC outputs are generated over three independent axes:
\begin{itemize}[leftmargin=2em]
