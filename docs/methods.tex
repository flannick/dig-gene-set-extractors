...
(ii) calibration methods (\texttt{contrast\_methods}) that adjust peak/gene signals relative to external baselines.
...
(3) optionally calibrate peaks/genes against references (contrast-method axis),
...
\texttt{--contrast} and related condition flags & Study-design contrast defining $x_p$ & Section \ref{sec:study_design_contrast} \\
\texttt{--peak\_weight\_transform} & Transform $\alpha_p=\phi(x_p)$ & Section \ref{sec:peak_weights} \\
\texttt{--contrast\_methods} & External calibration (peak-level or gene-level) & Section \ref{sec:contrast_axis}, Eqs. \ref{eq:ref_idf}, \ref{eq:atlas_logratio}, \ref{eq:atlas_z} \\
...
\item \textbf{Contrast-method axis} ($r$): how raw accessibility is calibrated against baseline/reference context.
CLI: \texttt{--contrast\_methods}. Output tag: \texttt{\_\_contrast\_method=...}
...
Before calibration by \texttt{--contrast\_methods}, users define a study-design contrast that produces peak statistics $x_p$:
\begin{itemize}[leftmargin=2em]
...
In output naming this study-design stage appears as \texttt{\_\_contrast=...}.

Study-design contrast defines $x_p$ first. The calibration contrast-method axis (\texttt{--contrast\_methods}) is applied after $x_p$ is defined, and adjusts peak or gene signals relative to external references.

\section{Contrast-method axis}
\label{sec:contrast_axis}
The contrast-method axis ($r$) defines how peak or gene signals are calibrated before extracting gene sets.
Conceptually, this axis answers: ``contrast against what baseline, and at what stage (peak-level or gene-level)?''

\subsection{Contrast method options}
\begin{itemize}[leftmargin=2em]
...
Program families are independent of link and contrast-method choices, and are evaluated for each active $(r,\ell)$ combination.
...
\item \texttt{contrast\_methods}: calibration/contrast choice (\texttt{none}, \texttt{ref\_ubiquity\_penalty}, \texttt{atlas\_residual}).
...
\end{itemize}
Each active contrast method is evaluated for each selected linkage model, and each resulting linked score space is then emitted through requested program families.
...
\item a single contrast outcome per pair using \texttt{--contrast\_methods auto\_prefer\_ref\_ubiquity\_else\_none} (prefer reference ubiquity calibration, otherwise fallback to \texttt{none}).
\end{itemize}
