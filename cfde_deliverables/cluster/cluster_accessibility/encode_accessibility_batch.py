...
for line in gzip.open(REFGENE,'rt'):
    ...
    if '_' in ch or not a.isdigit(): continue
    gene_tss[g].add((ch,int(a) if st=='+' else int(b)))
log(f"refGene genes: {len(gene_tss)}")

q=urllib.parse.urlencode({"type":"File","assay_title":ASSAY,"file_format":"bed","output_type":OUTPUT,
...
def accessible(p):
    ...
    for g,tsss in gene_tss.items():
        for c,t in tsss:
            cb=cov.get(c)
            if cb and any(bb in cb for bb in range((t-WIN)//BIN,(t+WIN)//BIN+1)): out.append(g); break
...
for bs,(acc,exp,href) in seen.items():
    ...
    try:
        ...
        open(os.path.join(dd,"genesets.gmt"),"w").write(f"{name}\tGenes promoter-proximal to ENCODE {ASSAY} peaks in {bs}\t"+"\t".join(genes)+"\n")
        json.dump({"standard_name":name,"library":LIB,"description":f"Genes with promoter-proximal ENCODE {ASSAY} accessibility in {bs} (DERIVED from ENCODE public peaks).","version":"0.1","file_type":"geneset","n_genes":len(genes),"organism":"human","assembly":"GRCh38","biosample":bs,"assay":ASSAY,"derived_in_this_work":True,"source":cite},open(os.path.join(dd,"geneset.meta.json"),"w"),indent=1)
